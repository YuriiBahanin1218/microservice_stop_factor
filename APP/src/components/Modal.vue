      ...
      <div class="modal-window" :style="`width: ${openInfo.width}px`">
        ...
        <v-row class="inputs-wrapper" id="iwrapper">
          <v-col :cols="input.col" v-for="(input, index) of openInfo.inputs" :key="(index+44) *3" class="d-flex justify-center" :style="`padding-left: ${input.pl ? input.pl : 0}; padding-right: ${input.pr ? input.pr : 0};`">
            ...
            <cAreaSource v-if="input.type === 'areaSource'" :item="input"></cAreaSource>
...
import CustomTextAreaSource from './inputs/CustomTextArea_source.vue'
...
export default {
  ...
  components: {
    ...
    cAreaSource: CustomTextAreaSource
  },
  data () {
    return {
      stopTitle: '',
  ...
  computed: {
    ...
    // ...mapGetters('source', ['getSourceName', 'getSourceDescription', 'getSourceDomain'])
  },
  methods: {
    ...mapActions('source', ['setName', 'setDescription', 'setDomain', 'saveSource']),
    closeModal () {
      // this.$store.dispatch('managerStops/clearFormulaAction')
      this.$store.dispatch('modal/toogleOpenAction', { open: false, inputs: [] })
    },
    saveData () {
      const modalName = this.$store.state.modal.modalName
      const sourceName = this.$store.getters['source/getSourceName']
      const sourceDescription = this.$store.getters['source/getSourceDescription']
      const domain = this.$store.getters['source/getSourceDomain']
      // this.$store.dispatch('managerStops/addNewStopAction')
      if (modalName === 'sourceModal') {
        console.log(modalName)
        if (sourceName && sourceDescription && domain) {
          this.$store.dispatch('source/saveSource', {
            name: sourceName,
